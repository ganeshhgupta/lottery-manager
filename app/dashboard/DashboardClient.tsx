...
export default function DashboardClient({ slots: initialSlots, closingLog: initialLog, catalog: initialCatalog }: Props) {
  ...
  // Slot picker — catalog has a setter so it refreshes after adds/deletes
  ...
  async function handleVerify(e: React.FormEvent) {
    ...
    try {
      // Verify credentials + fetch fresh catalog in parallel
      ...
      if (!authRes.ok) {
        setAuthError(authData.error ?? "Invalid ID or PIN");
        return;
      }
      if (catRes.ok) {
        const catData = await catRes.json();
        setCatalog(catData.catalog ?? catalog);
      }
      setVerifiedId(authData.employeeId); setVerifiedName(authData.employeeName);
  ...
  function openPicker(slotNumber: number) { setPickerSlot(slotNumber); }
  ...
  async function handlePickTicket(ticket: TicketCatalogItem) {
    ...
    try {
      const res = await fetch("/api/slots/assign", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ employeeId: verifiedId, pin: authPin, slotNumber: pickerSlot, ticketName: ticket.name, imageUrl: ticket.imageUrl }),
      });
      if (res.ok) {
  ...
  return (
    <div className="min-h-screen bg-slate-900">
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 min-w-0">
            <div className="flex items-center gap-1.5 flex-shrink-0">
              ...
              <span className="text-lg font-bold text-white hidden sm:inline">LotteryAudit</span>
            </div>
            {/* Action buttons — only when viewing latest in off mode */}
            {closingMode === "off" && isLatest && (
              <div className="flex items-center gap-2">
                <button
                  onClick={startClosing}
                  className="bg-indigo-600 hover:bg-indigo-500 text-white text-xs sm:text-sm font-medium px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap"
                >
                  + Closing Entry
                </button>
                {userInfo && (
                  <button
                    ...
                    className="bg-slate-700 hover:bg-slate-600 text-white text-xs sm:text-sm font-medium px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap"
                  >
                    Edit Ticket
                  </button>
                )}
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
            ...
            {userInfo ? (
              <>
                <span className="text-slate-300 text-sm hidden md:inline">{userInfo.employeeName}</span>
                <span className={`text-xs px-2 py-1 rounded-full font-medium hidden sm:inline ${userInfo.role === "manager" ? "bg-indigo-600/30 text-indigo-300 border border-indigo-600/50" : "bg-slate-700 text-slate-300"}`}>
                  {userInfo.role === "manager" ? "Mgr" : "Emp"}
                </span>
                {userInfo.role === "manager" && (
                  <a href="/manager" className="text-xs text-indigo-400 hover:text-indigo-300 transition-colors hidden lg:inline">Manager Panel</a>
                )}
                <button onClick={handleLogout} className="bg-slate-700 hover:bg-slate-600 text-slate-300 text-xs sm:text-sm px-3 py-1.5 rounded-lg transition-colors">Logout</button>
              </>
            ) : (
      ...
      <main className="max-w-7xl mx-auto px-4 py-4">
              ...
              {slots.map((slot) => (
                <div key={slot.slotNumber} className="flex flex-col gap-1">
                  ...
                  >
                    ...
                    {/* Swap button — top right, bigger for easy tap */}
                    <button
                      type="button"
                      onClick={() => openPicker(slot.slotNumber)}
                      className="absolute top-1 right-1 bg-black/75 hover:bg-indigo-600 text-white text-xs sm:text-sm px-1.5 py-0.5 rounded-md transition-colors leading-none font-bold"
                      title="Change ticket"
                    >
                      ⇄
                    </button>
        ...
        {closingMode !== "edit" && (
          <>
            {/* Compact entry navigator */}
            <div className="bg-slate-800 rounded-xl border border-slate-700 px-4 py-3 mb-4 flex items-center gap-3">
              <button
                onClick={() => setVersion((v) => Math.max(0, v - 1))}
                disabled={version === 0}
                className="text-slate-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed text-lg px-1 transition-colors flex-shrink-0"
              >
                ‹
              </button>
              ...
              <div className="flex-1 min-w-0 text-center">
                {currentEntry ? (
                  <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-0.5">
                    <span className="text-white text-sm font-medium">{currentEntry.date}</span>
                    <span className="text-slate-400 text-sm">{currentEntry.shift}</span>
                    <span className="text-slate-400 text-sm">{currentEntry.employeeName}</span>
                    {isLatest && (
                      <span className="text-indigo-400 text-xs font-medium bg-indigo-500/10 border border-indigo-500/30 px-2 py-0.5 rounded-full">current</span>
                    )}
                  </div>
                ) : (
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <form onSubmit={handleVerify} className="bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl p-6 w-full max-w-xs flex flex-col gap-4">
            ...
            <div>
              ...
              <input type="text" inputMode="numeric" pattern="[0-9]*" value={authEmpId}
                onChange={(e) => setAuthEmpId(e.target.value)} required autoFocus
            ...
            <div>
              ...
              <input type="password" inputMode="numeric" pattern="[0-9]*" value={authPin}
                onChange={(e) => setAuthPin(e.target.value)} required maxLength={4}
            ...
            {authError && <p className="text-red-400 text-sm text-center -mt-1">{authError}</p>}
      ...
      {pickerSlot !== null && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-3xl max-h-[80vh] flex flex-col">
